...
def make_seesion(FLAGS):
  global train_writer,test_writer
  mnist_sess = tf.Session(graph=tf.get_default_graph(),config=tf.ConfigProto(log_device_placement=False,
...
def apply_placement():
    #with mnist_sess.as_default():
  	tf.global_variables_initializer().run(session=mnist_sess)
def train():
  #global train_writer,test_writer
  # Import data
