...
Status Placer::Run() {
  ...
  for (auto &it : devices_->devices()){
    //cout<<it->name()<<endl;
    graph_->InternDeviceName(it->name());
  }
  //exit(0);
  //graph_->InternDeviceName(devices_->devices()[0]->name());
  //graph_->InternDeviceName(devices_->devices()[1]->name());
  for (Node* node : graph_->op_nodes()) {
    //open file of clustering and assign clusters
    //fist line is device index
    //next line is nodes id
    //cout << node->name()<<" "<<node->id()<<endl;
    int assigned_device = 1;
    //cout << devices_->devices()[0]->name()<<endl;
    //cout << devices_->devices()[1]->name()<<endl;
    //cout << devices_->devices()[2]->name()<<endl;
